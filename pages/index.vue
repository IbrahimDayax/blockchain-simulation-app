...
<script>
import { ref, reactive, computed, defineComponent, onMounted, useContext } from '@nuxtjs/composition-api'
...
export default defineComponent({
  setup() {
    ...
    const startWebsite = () => {
      ...
      // const from = wallets.value[0]
      // const to = wallets.value[1]
...
function useUI({ blockchain, wallets }) {
  ...
  const tabs = computed(() => {
    ...
    const walletsCount = wallets.value.length
  ...
  const componentTab = computed(() => {
    ...
    switch (activeTab.value) {
      case 1:
        component = () => import('@/components/Wallets.vue')
        break;
      case 2:
